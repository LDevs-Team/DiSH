...
async def dump(client, message: discord.Message, args: str, send):
    """
    ...
    if not os.path.exists(args):
        return await send("Not existing-file")

    if os.path.isdir(args):
        executor = functools.partial(shutil.make_archive, args, "zip", args)
        await client.loop.run_in_executor(None, executor)
        return await send(file=discord.File(args + ".zip"))

    with open(args, "rb") as fp:
        await send(file=discord.File(fp))


async def cd(client, message: discord.Message, args: str, send):
    """
    ...
    print(args)
    os.chdir(args)
    await send("Changed directory to " + args)


async def upload(client, message: discord.Message, args: str, send):
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(file) as r:
            ...
            with open(args, "wb") as fp:
                fp.write(data)
                await send("Uploaded file to " + args)
                fp.close()
            r.close()
        await session.close()


async def download(client, message: discord.Message, args: str, send):
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as r:
            ...
            with open(" ".join(args.split(" ")[1:]), "wb") as fp:
                fp.write(data)
                await send("Downloaded file to " + " ".join(args.split(" ")[1:]))
                fp.close()
            r.close()
        await session.close()


async def edit(client, message: discord.Message, args: str, send):
    """
    ...
    :type args: str
    """
    await send("Editing file " + args)
    try:
        await send(file=discord.File(args))
    except:
        await send("File does not exist :C")

    await send("Send the new file content")
    try:
        ...
        f.close()
        await send("Edited file")
    except asyncio.TimeoutError:
        await send("Too much time has passed :C")


async def pwd(client, message: discord.Message, args: str, send):
    """
